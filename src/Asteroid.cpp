...
Asteroid::Asteroid()
{
    ...
    mFriction = 1;
    mPosition = SetRandomSpawnPosition(SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2, 250);
...
Vector2f Asteroid::SetRandomSpawnPosition(float centerX, float centerY, float radius)
{
    float randomAngle = static_cast<float>(rand() % 360);
    float angleInRadians = ToRad(randomAngle);
    float newX = centerX + radius * cos(angleInRadians); 
    float newY = centerY + radius * sin(angleInRadians); 
